...
Not sure how to do this. We would need the time metric in wide format for this -- and at this point, we only have the wide format for the item number -- just checking. 
